...
export default class MirrorRequestScanner extends Scanner {
  constructor(options: ConfigOptions) {
    ...
    console.log(
      `starting mirror request scanner at ${options.scanInterval}ms interval`
    );
  }
      ...
      .then(videos => {
        videos.forEach(vid => {
          ...
          vid
            ...
            .then(() => {
              console.log(
                `download for ${vid.redditPostId} finished, beginning upload`
              );
              vid
                .upload()
                .then(() => {
                  console.log(
                    `successfully uploaded video ${vid.redditPostId}`
                  );
                })
                .catch(err => {
                  console.error(`${err}`);
